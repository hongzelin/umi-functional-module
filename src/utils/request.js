...
function checkCode(response) {
  ...
  if (`${response.errCode}` === "-1" || `${response.errCode}` === -1) {
    ...
    if (response.data && response.data.loginUrl) {
      const newUrl = response.data.loginUrl;
 ...
 * @param {*} options 请求参数
 */
export default function request(url, options, isForm) {
  const _header = {
  ...
  return fetch(url, newOptions)
    ...
    .catch(err => {
      ...
      if (status === "-1") {
        router.push(`/exception/500?msg=${err.response}`);
      }
 ...
 * 统一处理URL 添加前缀等统一操作
 * @param {*} url 原URL
 * 可以考虑把固定部分抽离成配置文件，根据需求来 可以调整apis,也可以在.umirc.js中配置pathRewrite，apis
 */
const trimURL = url => `/apis${url}`;
